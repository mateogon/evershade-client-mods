...
for(const file of files){
 if(!/^(?:[^/]+\.(?:js|ps1|md)|portable\.xml|\.gitignore|\.gitattributes|patches\/(?:client\.patch|baseline\.json)|profiles\/(?:LeaderPlayer|FollowerOne|FollowerTwo)\.xml|drops\/(?:app\.js|index\.html)|build\/network-probe\/(?:NetworkProbe\.as|probe\.xml))$/.test(file))issues.push(file+': unexpected tracked path');
